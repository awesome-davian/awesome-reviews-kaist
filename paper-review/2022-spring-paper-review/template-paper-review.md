...
$$
여기서 $S_i(x) = x/2^i$는 input image $x$에 대한 scale copy를 나타냅니다. Scale image가 어떻게 여러 모델을 학습시키는 model augmentation 효과를 보이는지에 대한 논리 전개와 증명은 [**원본 논문**](https://arxiv.org/pdf/1908.06281.pdf)에 자세히 서술되어 있으니 참고하시길 바랍니다.
