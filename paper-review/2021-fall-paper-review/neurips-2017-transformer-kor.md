...
이전까지 Sequence data를 처리하기 위해서는 Recurrent Model을 이용해 __output<sub>t</sub>__ 을 위해 __input<sub>t</sub>__ 와 __hidden<sub>t-1</sub>__ 를 사용한다. 
...
__x<sub>1</sub>, ... , x<sub>n</sub>__ 을 input sequence, __y<sub>1</sub>, ... , y<sub>n</sub>__ 을 output sequence, __(h<sub>1</sub>, h<sub>2</sub>, ... , h<sub>n</sub>)__ 을 입력 시퀀스 hidden vector라고 합시다. Decoder의 i 번째 hidden state를 __s<sub>i</sub>__ 라 하면, 다음과 같이 계산 된다.

$$ s_i = f(s_{i-1}, y_{i-1}, c_i) $$
$$s_{i-1}: i-1번째 hidden state.$$
$$y_{i-1}:i-1번째 output word.$$
