...
이 섹션에서는 이 모델이 어떻게 동작하고 어떠한 방식으로 학습이 되는지를 이해해봅시다.
...
* 우리가 학습과정에서 볼 수 있는 건 뭘까요?
  ...
  * ImageNet에서 사전학습(pretrained)된 weight를 지니고 있는 Encoder 만을 볼 수 있습니다. 

* 우리 모델은 어느 데이터셋으로 성능을 평가받나요? 
...
이 논문의 가장 주가 되는 방법은 push and pull  입니다

* Pull: synthetic데이터의 피쳐와 ImageNet-pretraiend 피쳐 간의 거리를 minimize 합니다.
* Push: synthetic 도메인에서 이미지들의 피쳐들이 서로서로를 밀어내어 어느정도의 거리를 만들게 합니다.

ASG모델 (Fig.3-a) 와 비교하면, 본 논문의 아키텍쳐 (Fig.3-b) 는 아래와 같습니다. 
...
일반적으로 $$\alpha=0.99 , \beta=0.01$$ 로 셋팅을 하는데요, exponential moving average function은 특이하게도 $$k $$ 가 증가함에 따라 $$\beta$$ 의 값이 감소합니다 (e.g., 처음엔 0.01 이었다가 나중엔 0.001 이 되는 거죠). 이렇게 되면 모델로 하여금 현재 다루고 있는 데이터에 집중을 하고 과거의 데이터에는 신경을 덜쓰게 합니다.

우리는  $$W_0 \to \tilde{h}$$ , $$W_k \to h$$  라고 이해할 수 있을 것 같습니다. 즉, ImageNet (과거 데이터)에 사전 학습된 피쳐 임베딩의 정보를 조금씩 synthetic dataset (현재 데이터)에 맞게 튜닝을 시키는 것이죠.
...
여러 contrastive learning 방식 중이서, 본 논문은 InfoNCE loss를 사용합니다 (Related work 섹션에서 이미 다뤘습니다).
...
하지만, synthetic image의 경우 배경이 없고 물체 하나만 있는 경우가 많기때문에, 이렇게 average pooling을 하게 되면 의미없는 배경 벡터(예를 들어 흰 배경)까지 같이 더하게 되어, 오히려 물체의 특성을 희석시키게 됩니다. 
