...
$$\overrightarrow{Op} \dot [\vec{OO'} \times \vec{O'p'}]$$
...
$$\hat(p)^TF\hat(p)' = 0 with F = K^(-T)EK^('-1)$$
...
여기서 구해야 하는 값은 $$f_(11) ~ f_(33)$$으로 총 9개이지만 단안카메라의 경우 up to scale이기 때문에 8개의 값을 구해야 하며, 그렇기 때문에 총 8개의 방정식이 필요합니다.
